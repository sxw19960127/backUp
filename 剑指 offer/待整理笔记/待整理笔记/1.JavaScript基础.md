...
1.简介
	...
	js本身并没有提供输入输出的语句,必须依赖于当前所存在的环境为他提供输入输出的语句;
...
------------------------------------------------------------
2.数据类型区分依据
	根据: 1)存储空间大小的差异; 2)所支持的操作符的差异;
3.基础类型/引用类型/特殊类型
	1)基础类型
		Number数字 64bit
				1.1)按数字特殊值划分;
					isFinite()判断
                	isNaN()判断: NaN和任何数(包括他自己)进行操作结果都是false
        String字符串 16bit(大多数字符)
            	1.4)转义字符;
            		\n 换行字符
           			\r 回车字符
            		\\ 输出\
	...
	3)特殊类型
		...
		undefined
			3.1)声明的变量未赋值
    		3.2)未给方法参数赋值,则对应参数为undefined
    		3.3)方法未明确返回值,则执行方法的结果为undefined
-------------------------------------------------------
	var a = new Number(1);
...
4.数据类型之间的转换
	数字/布尔值 ---> 字符串
		...
		var a = String(true); 
	字符串 ---> 数字
		var a = parseInt("10"); //a = 10;
			a = parseInt("abc"); //a = NaN;
		var a = Number("3.14"); //a=3.14;
	布尔值 ---> 数字
		var a = Boolean(true); //a=1;
-------------------------------------------------
5.typeof
以"字符串"的形式返回指定变量的数据类型
	typeof 1 === "Number"
	typeof "abc" === "String"
	typeof true === "Boolean"
	typeof Null === "Object"
...
4)逻辑运算符
	！ (取反)
    && (并且,有一个是false结果就是false)  
    || (或者,有一个是true结果就是true)  
    	注:优先级 ！ >  &&  >  ||
    var a = true && false && false; //false
    var a = 1 > 3 && 4 < 5 || 1 == "1"; //true
    短路:
    	var a = 1 > 2 && 3.1231*5 > 19.85; //false
    	Var b = 31e2334 > 2341 && 13141 < 3242 || true; //true
----------------------------------------------------------------
8.语句
	1)分支语句
        A)if-else语句
			...
			注:如果if/else代码块中仅有一条语句,则可以省略大括号
			注:false,空字符串,0,null,undefined 写在if中是false,其余是true
			注:三元表达式(A?B:C) 
				如果A为true,表达式的值为B,否则为C;
				三元表达式在特殊情况下可以代替if-else;
            var max3a = function(a,b,c) { //三个数比较大小
                var max = a > b ? a : b;
                return max > c ? max : c;
            };
            var max3b = function(a,b,c) { //简化,一步到位
                return a > b ? (a > c ? a : c) : (b > c ? b : c);
            };
		B)switch-case-default语句
            switch(数字) {
                case 值1: //几率大的case写在前面,利用穿透性
					break; //立即退出循环,仅能退出一层
                case 值2:
                case 值3:
                ...
		...
		说明:如果switch后面的数字值与下面的case后面的值相同,则执行case后面"所有"的代码;
			即:穿透性,但在对应case后面加上break以后,便不执行后面的代码;
		default表示除了上面值之外的其他值,当然default也可以不写;
	2.循环语句
	while循环
		while(A) {
            B:代码
		}
		若A为true,执行B,再回去判断A,...直到A为false停止流程
	...
	do-while循环:
		...
		执行逻辑: 开始先执行B,再判断A,A为true,则再执行B,否则循环结束;
	for循环:
        for(A; B:布尔值 ;C) {
            D:循环体
        }
